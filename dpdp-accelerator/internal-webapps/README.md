...
Nothing lives here yet. The consent portal is a `react-apps` module rather than an internal webapp
because it ships a user-facing SPA alongside its backend-for-frontend; this directory is for
server-side endpoints with no UI.

This directory has **no aggregator `pom.xml`**. Add each
